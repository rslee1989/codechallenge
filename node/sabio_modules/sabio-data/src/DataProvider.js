const { ConnectionPool, TYPES } = require("mssql");
...
const proDebug = debug("sabio:data:provider");
const isPoolDebugEnabled = debug.enabled("sabio:data:pool");

const pool = configurePool();

function DataProvider(connString, opts) {
...
async function configurePool(connectionString, opts, onError) {
  ...
  function onCtroErrorLocal(err) {
    proDebug("Ctro Error in Data Provider");
    proDebug(err);
    if (onError) {
      onError(err);
    }
